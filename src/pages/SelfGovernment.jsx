  ...
  return (
    <section className="container">
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {selfgovermentList.map((s) => (
          <GovermentItem goverment={s} key={s.label} />
        ))}
      </section>
    </section>
  );
  ...
  return (
    <Drawer>
      <DrawerTrigger className="text-center cursor-pointer flex-col shadow-xl gap-4 flex rounded-lg p-5 border border-border">
        <object
          data={fileUrl + s.img}
          type={`image/${s.img.split(".")[1]}`}
          className="w-30 rounded-lg mx-auto aspect-square"
        >
          <img
            src={imgFallback}
            className="rounded-lg mx-auto aspect-square"
            alt={s.label}
          />
        </object>
      ...
      <DrawerContent className={"min-h-[95%] "}>
        ...
        <section className="overflow-auto px-5 pb-10  flex flex-col gap-5">
          <section className="p-5 border border-border rounded-lg mb-5 flex gap-10 shadow-xl flex-col md:flex-row">
            <object
              data={fileUrl + s.leaderImg}
              type={`image/${s.img.split(".")[1]}`}
              className="aspect-[3/4] rounded-lg max-h-96"
            >
              <img
                src={imgFallback}
                className="aspect-[3/4]  w-full max-h-96 rounded-lg"
                alt={s.label}
              />
            </object>
            <section className="flex text-lg gap-5 flex-col justify-center">
              ...
              <a className="flex gap-2" href={`tel:${s.number}`}>
                <PhoneIcon /> {s.number}
              </a>
              {s.email && (
                <a className="flex gap-2" href={`mailto:${s.email}`}>
                  <Mail />
                  {s.email}
                </a>
              )}
          ...
          <section className=" py-5 px-3 lg:pl-10 border border-border shadow-xl rounded-lg ">
            <p
              ...
              dangerouslySetInnerHTML={{
                __html: formatMessage({ id: s.label + ".content" }),
              }}
            ></p>
          ...
          <section className="grid md:grid-cols-2 gap-5 lg:grid-cols-3 ">
            <FileCard
              name={"selfgoverment.plan"}
              link={s.label.replace(".", "/") + "plan.pdf"}
            />
            <FileCard
              name={"selfgoverment.studentlist"}
              link={s.label.replace(".", "/") + "list.pdf"}
            />
            <FileCard
              name={"selfgoverment.report"}
              link={s.label.replace(".", "/") + "report.pdf"}
            />
          </section>
        </section>
      </DrawerContent>
